...
#[rustfmt::skip]
LoadDefinitions!({
  // Perl ar5iv-bindings/phyzzx.tex.ltxml — phyzzx HEP format, full ~2440
  // line version (source cited: http://arxiv.org/macros/phyzzx.plus).
  // Perl port is one big RawTeX heredoc + 2 trailing DefMacro entries
  // (\eqn, \leqn) — Rust mirrors the same shape. Uses r##"..."## raw
  // delimiter because the TeX body contains `"#` sequences (e.g.
  // `\mathchar"#1` on L335).
  RawTeX!(r##"
\let\rel@x=\relax
\let\n@expand=\relax
...
"##);
  ...
  // Perl L2459-2460: two DefMacros with locked=>1
  DefMacro!("\\eqn Token",
    "\\advance\\equanumber by 1\\relax\\xdef#1{\\the\\equanumber}",
    locked => true);
  DefMacro!("\\leqn Token",
    "\\advance\\equanumber by 1\\relax\\xdef#1{\\the\\equanumber}",
    locked => true);
