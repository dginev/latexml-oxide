...
#[rustfmt::skip]
LoadDefinitions!({
  // Perl ar5iv-bindings/crckapb.sty.ltxml — CRCKAPB document style for
  // Kluwer Academic Publishers Books (source: https://arxiv.org/macros/
  // crckapb.sty). The Perl port is essentially one large RawTeX block
  // wrapping the raw .sty contents. This Rust port mirrors that shape —
  // a ~875-line RawTeX! block plus the three top-level dependencies.
  LoadClass!("book");
