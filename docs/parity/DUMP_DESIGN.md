...
> definitions as Perl.
>
> **Known gap (2026-07-17): ownership inversion.** Rule 1 makes the dump the
> `base` layer, but it is generated by raw-loading REAL `latex.ltx`, which
> overwrites LaTeXML's own reimplementations — so the dump encodes what real
> LaTeX defines, not what base defines, in slots LaTeXML owns (e.g. Perl's
> `Let('\@popfilename','\lx@popfilename')`). Patched with an accreting blocklist
> (`dump_writer::LATEXML_HOOK_STUBS`); the invariant that would retire it is
> filed in [`SYNC_STATUS.md`](../SYNC_STATUS.md) → "Dump ownership inversion".
