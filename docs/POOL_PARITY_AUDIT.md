...
| `TeX_Math.pool.ltxml` | `engine/tex_math.rs` | ✅ walked — early section aligned: `\nonscript`, `Tag(ltx:XMText)`, `\lx@dollar@default`, `Let(T_MATH, ...)`, `\lx@dollar@in@*mode` Lets, `\lx@dollar@in@mathmode`, `\lx@begin/end@inmath@text`, `\lx@begin/end@display@math`, `\lx@begin/end@inline@math`, `Tag(ltx:Math, after_close)` (×2). Rust file is much larger (2117 vs 1277) due to inlined helpers but Perl-side calls remain in the same sequence. Late sections (`\lx@*@superscript/subscript`, math rewrites) similarly aligned. |
...
| `plain_bootstrap.pool.ltxml` | `engine/plain_bootstrap.rs` | ✅ walked — `\TeX` constructor, `\alloc@`, `\ch@ck`, `\newif`, `\leavevmode` all in Perl plain_bootstrap order. Rust extras (CSS-styled `\TeX` logo) consistent with Perl L19-27. |
| `plain_base.pool.ltxml` | `engine/plain_base.rs` | ⚠️ walked with notes — early order diverges: Rust front-loads `\magnification` register (Rust extra, not in Perl plain_base), then `\loggingall`/`\tracingnone`. Perl starts directly with `\loggingall` (L30). Several macros also reorganized into `\#`/`\&`/`\$` `\ifmmode` dispatchers (Rust) vs simple `DefPrimitive` (Perl L72-76). Documented for future surgical reorder. |
| `math_common.pool.ltxml` | `engine/math_common.rs` | ✅ walked — DefMath sequence for Greek letters (`\alpha`, `\beta`, `\gamma`, ..., `\vartheta`, ...) in Perl L21+ order. Rust starts the LoadDefinitions block at L159 (top has helper closures); the math defs themselves match line-by-line. |
| `latex_base.pool.ltxml` | `engine/latex_base.rs` | ⚠️ walked with notes — Rust front-loads `\@empty`/`\@begindocumenthook`/`\@elt` Lets and `\@qend`/`\@qrelax`/`\@spaces`/`\@sptoken`/`\@topnewpage` defs that don't appear at the top of Perl latex_base. Perl L36-117 starts with `Let \@popfilename`, `\@gobbletwo`, `\@firstofone`, `\@firstoftwo`, `\@thirdofthree`, `\@makeother`, `\@width`/`\@minus`. Substantial reorder candidate (similar magnitude to eTeX). |
