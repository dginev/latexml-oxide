...
- **Scanner-status residuals** (batch 56jg, DIVERGENCES #310): `\message`/`\errmessage`/`\mark`/`\special` keep Perl's stop at a file end (respell them `GeneralText`, as tex.web's `scan_toks`); commands Rust implements as primitives/constructors (`\setcounter`, `\lstnewenvironment`, `\newtcbinputlisting`) need a primitive-level abort for TeX's matching recovery; `skipping` is not modelled; `\toks0=\bgroup abc` is not accepted as a token assignment (TeX takes the implicit brace, §1226).
