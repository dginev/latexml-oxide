...
- **Tokens after a `{Dimension}`/`{Glue}` argument's scan vanish silently** (56jm review, pre-existing; content loss): `A\hspace{1em\foo}B` gives "A B" (pdflatex "Ax B" with `\def\foo{x}`), `\setlength{\parindent}{2pt\foo}G` gives "G" (pdflatex "xG"); `\makebox[\widthof{ab}+\widthof{…}]` drops the `+…` term (10.6pt vs pdflatex 39.0pt) with no diagnostic. TeX leaves the rest in the input.
- **The case changer expands `\newcommand` optional-argument commands** (56jm review): `\MakeUppercase{a\foo b}` with `\newcommand\foo[1][x]{#1}` gives "AXB", pdflatex "AxB" — l3text leaves a `\@protected@testopt` command unexpanded (expl3-code.tex:36319-36328); `Parameter::testopt` (56jm) is the flag the case changer needs.
