...
| — | **Streaming CORE (fragmented conversion)** | **IMPLEMENTED on `feat-streaming-xml-core`** (2026-07-29, one PR pending): stomach yield → spill/placeholder → pass 2 → splice; `--streaming` + auto-when-doomed; byte-identical to eager (suite 1781/0, 8 sweep suites, 19.8 MB witness `cmp`-equal at 613,104,457 B). **Plus the leak fix that halved eager**: rust-libxml unlink never frees; `Node::free_subtree` (libxml 0.3.17) + `Document::discard_subtree` → witness eager 63.7→34.9 GB, streamed@24 GB-cap completes at 16.97 GB/8:28 (was SIGKILL). Token backstop now scales ×200/byte for book-scale sources. Fork publish 0.3.17 + dep bump pending (temporary `[patch]` in Cargo.toml). | — | [`STREAMING_CORE_DESIGN_2026-07-29.md`](performance/STREAMING_CORE_DESIGN_2026-07-29.md) |
| — | Two-pass streaming split (POST half) | **deferred by user decision 2026-07-06**; trigger arrived (Nasser 131 MB) — next after the core PR | — | [`STREAMING_POST_DESIGN_2026-07-06.md`](performance/STREAMING_POST_DESIGN_2026-07-06.md) |
