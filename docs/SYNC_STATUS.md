...
- [ ] **38. xytest** (65_graphics) — 20 errors (was 102 TooManyErrors). Session 46: mouth double-finish catcode fix, at_letter infrastructure, xy.tex.ltxml overlay, xylatexml SVG driver, SVG namespace serialization fix (get_node_qname + serializer prefix), option processing fix (@currname save/restore). Extensions (matrix/arrow/curve/knot) now load. Remaining: SVG schema validation (svg:g content model), coordinate values all zero (range registers).
