...
- [ ] **56. babel suite** (81_babel) — down from 202 to ~75 diffs (csquotes sub-test). Session 47: fixed \@fontenc@load@list format (`\@elt{T1,OT1}` → `\@elt{T1}\@elt{OT1}`) which caused babel's AtBeginDocument font encoding iteration to leak commas. Added `\def\@fontenc@load@list{}` after babel load as safety clear. Stray commas eliminated for minimal babel. Remaining: (1) quote chars `?` fallback (csquotes can't find language styles because \bbl@main@language is "nil"); (2) xml:lang on document root; (3) batch test hangs on one sub-test (greek or page545).
