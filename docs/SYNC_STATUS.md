...
- [ ] **56. babel suite** (81_babel) — OOM on \selectlanguage. Session 47: fixed \@fontenc@load@list comma leak. Root cause of OOM: `\selectlanguage{french}` → `\bbl@provide@locale` → `\babelprovide{french}` loads French .ini data. In Perl (precompiled kernel), languages already loaded, heavy path skipped. Our fresh kernel hits full ini-loading which causes 14-26GB memory usage. BLOCKED on precompiled kernel (E).
