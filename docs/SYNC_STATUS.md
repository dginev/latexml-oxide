...
| **R9** | Deep deferred families (`.bst`, xy-pic, mode-frame, …) | parked, EXCEPT the **mode-frame family, un-parked by user directive 2026-09-02** ("all queued surpass shapes + R9 approved") and worked on the `perfect_kernel` branch as the `boxes-groups`/`alignment` topics of `tools/perfect_kernel/repros/` (design brief: `perfect_kernel/ARCHITECTURE_THEMES.md` §1 incl. the 2026-09-03 correction — a `$`/`}` meeting the wrong group stays a TeX error, tex.web §1065); the rest still carry explicit "do NOT start". The `.bst` row's "`.bst` files *vendor macro definitions*" premise was **RETRACTED 2026-07-27** (`alpha.bst` has zero `Dbar`; the macro is `mathscinet.sty`'s) — it survives on label style / sort order / **field selection**, and the prerequisite is a corpus measurement of the `.bib`+`.bst`-with-no-`.bbl` population | **family** | [`DEFERRED_FAMILIES.md`](parity/DEFERRED_FAMILIES.md), and R9-BST below |
