...
- [x] **E. Precompiled kernel dump** — **WORKING END-TO-END**: `latexml_oxide --init=latex.ltx --dest=latex_dump.oxide` processes latex.ltx and writes 5,963 entries (1.1MB). `LATEXML_DUMP=latex_dump.oxide cargo test` loads the dump at runtime (5,842 entries, 0 errors). 308/310 tests pass with dump. Infrastructure: dump_writer.rs (serializer), dump_reader.rs (deserializer), dump_loader.rs (Perl format), ini_tex.rs (--init mode), State::snapshot/diff. Add-only policy prevents overriding Rust engine definitions.
