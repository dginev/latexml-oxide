...
### Dump ownership inversion + preload/documentclass divergence (2026-07-17) — OPEN, needs an investigation

**Symptom.** `--preload=<any>.cls` prints `LaTeX hooks Error: Extra \PopDefaultHookLabel`
(article/book/report; `.sty` preloads are clean). Perl is silent for the same preload, and
our own `\documentclass{article}` is clean. `LATEXML_NODUMP=1` is clean → the dump is the
source. Visible on every `latexmlmath_oxide` run, which preloads `article.cls`.

**Cause.** `DUMP_DESIGN` rule 1 makes the dump the `base` layer, but it is generated by
raw-loading REAL `latex.ltx`, which overwrites LaTeXML's own reimplementations during
`--init`; those overwrites get captured. Perl `latex_base.pool.ltxml` L35-36 does
`Let('\@popfilename','\lx@popfilename')` and we port it 1:1 (`latex_base.rs` L91-92) — but
the dump's REAL `\@popfilename` wins, and it calls `\__hook_curr_name_pop:`, which raises
the error when the label stack is empty. `\documentclass` escapes only because it routes
through `\@pushfilename` and balances the push. So the dump encodes *what real LaTeX
defines*, not *what base defines*, in slots LaTeXML owns.

**A blocklist does NOT work — measured, do not retry blind.** Filtering the filename stack +
L3-hook stubs at dump write/read silences the error and leaves preloads clean (0 Error/Fatal,
no undefined CS), but **regresses `cluster_mhchem_cf_author_macro` from 0 to 1003 errors**
(suite 1581/0 → 1572/9). Because the dump REPLACES base, filtering a slot does not fall back
to `latex_base.rs` — it leaves a HOLE. That kills the "just exclude LaTeXML-owned slots"
shortcut and answers the open question below: bindings DO rely on the dump. Also note
filtering `\PopDefaultHookLabel` alone is useless — the erroring caller is the expl3
internal `\__hook_curr_name_pop:`, not the public CS.

**Second divergence, same area.** The two load paths disagree on the class PI:
`\documentclass{article}` → `<?latexml class="article"?>`, `--preload=article.cls` →
`<?latexml class="article.cls"?>`. **Perl emits `class="article"` for BOTH.** Otherwise the
two outputs are byte-identical, so the preload DOES load `article_cls.rs` correctly — this is
just the name that reaches the PI. `parse_preload_spec` splits correctly to
`("article","cls")`, so the extension is re-attached further in.

**Where to look.** Either (a) make the preload path mirror `\documentclass` — same
`\@pushfilename`/`\@popfilename` balance and the same class name — which fixes both
divergences with no dump surgery; or (b) the deeper invariant: decide per-slot who owns a
definition (LaTeXML vs the raw load) and make dump generation reflect it, with a CI gate.
(a) is the smaller, better-scoped start and is what the symptom actually asks for.
