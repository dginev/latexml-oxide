...
* **FIXED 2026-05-29 — xy-pic `\crvi` cluster (curved arrows).** `\input xypic` collapsed onto `xypic_sty` did `RequirePackage("xy")` → marked xy.sty loaded → doc's `\usepackage[all]{xy}` option-clash dropped `[all]` → xycurve.tex unloaded → `\crvi` undefined. Fix: new `xypic_tex.rs` mirrors Perl `xypic.tex.ltxml` (`InputDefinitions('xy',tex)`, no RequirePackage) + idempotency guard on `\xyoption` redefinition in `xy_sty`. Flips 1603.04650, 1704.02401, 1804.00017, 2011.01105, 2012.03982.
* **FIXED 2026-05-29 — SciPost `\bra`/`\ket` undefined.** `scipost_cls.rs` OmniBus stub omitted SciPost.cls L53 `\RequirePackage{physics}` (Dirac notation). Fix: add `RequirePackage!("physics")`. Flips 2104.02751.
* **FIXED 2026-05-29 — algorithm2e `\nl` "ltx:tags not allowed in ltx:text".** `\algocf@printnl` emitted `<ltx:tags>` at cursor (inside KwInput's `<ltx:text>`) instead of floating out. Fix: port Perl L210 — `float_to_element("ltx:tags")` → emit → restore saved node. Flips 2104.02680.
* **FIXED 2026-05-29 — svproc `\apj`/`\sovast` undefined.** `svproc_cls.rs` omitted svproc.cls's inline AAS journal-abbrev set (L1387-1466, 79 macros). Fix: `RequirePackage!("aas_macros")`. Flips 2110.04152.
* **FIXED 2026-05-29 — biblatex-chicago `\lositemsep` undefined (incomplete-binding).** `\usepackage[notes]{biblatex-chicago}` → `\RequirePackage[style=chicago-notes]{biblatex}`; the `style=` option makes biblatex load chicago-notes.bbx which declares `\newlength{\lositemsep}` (L22), then biblatex-chicago.sty L154 `\setlength`s it. Rust's biblatex binding doesn't implement `.bbx`/`.cbx` style-file loading (Perl raw-loads the whole chain), so `\lositemsep` was undefined → `\setlength` + `<variable> expected` cascade. Fix: declare `\lositemsep` defensively in biblatex_sty.rs's length block (biblatex-chicago always loads biblatex). Flips 1802.09944, 1803.02857. Surfaced via current-binary cortex_worker re-test of stale stage-51-73 CONVERR.
* **FIXED 2026-05-29 — lmcs `\includegraphics` undefined (incomplete-stub).** lmcs.cls L24 `\usepackage{helvet,cclicenses}` pulls graphicx transitively: cclicenses→`\RequirePackage{rotating}`(L25)→`\RequirePackage{graphicx}`(rotating.sty L62). Perl ships no lmcs binding → raw-loads lmcs.cls → graphicx loaded; LMCS papers thus use `\includegraphics` WITHOUT their own graphicx load. Rust's `lmcs_cls.rs` stub (intercepts the raw cls, which fails on its tikz/XeTeXLinkBox ORCID machinery) omitted the transitive graphicx → `\includegraphics` undefined where Perl clean. Fix: `RequirePackage!(graphicx)` in the stub. Flips 1607.04128 RUST 1→0. Surfaced via systematic current-binary re-test of stale stage_60 CONVERR (46/67 still fail; harvested the ERR=1 subset). DEFERRED RUST-only: 1608.00275 (revtex4, anonymous `unexpected:_` at tex_math.rs:234, PERL 0 — needs `_`-source trace). spie.cls L92 does `\RequirePackage[superscript]{cite}` (cite.sty defines the no-brackets `\citen`/`\citenum`/`\citeonline`); SPIE templates use `\citen` directly. Rust's `spie_cls.rs` (LoadClass OmniBus) omitted cite — and OmniBus autoloads natbib's `\citenum` but NOT cite's `\citen`. Perl raw-loads spie.cls → cite → clean. Fix: `RequirePackage!("cite", ["superscript"])` in spie_cls (faithful — class genuinely loads cite). Flips 1808.10428. Surfaced via the current-binary sweep stage_74 logs.
* **FIXED 2026-05-29 — three stage_61 ERR=1 incomplete-binding clean wins.** Surfaced via systematic current-binary re-test of stale stage_61 CONVERR (50/63 still fail; gated the 14 ERR=1 papers vs Perl → 3 RUST-only). (1) **wasysym `\hexagon`/`\varhexagon`** + **wlscirep wasysym load** (1610.05398, wlscirep): wasysym.sty.ltxml L98-102 defines the box/shape glyphs (`DefPrimitiveI`); Rust's wasysym binding omitted `\Square`/`\XBox`/`\CheckedBox`/`\hexagon`/`\varhexagon`. Added them. AND wlscirep.cls L28 `\RequirePackage{wasysym}` — Rust's wlscirep stub mirrored most cls deps but omitted wasysym, so it never loaded. Added `RequirePackage!(wasysym)`. RUST 1→0 (`\hexagon` used in math as lattice-type label). (2) **IEEEAerospaceCLS `\acknowledgments`** (1610.07252): cls L290 `\def\acknowledgments{\section*{Acknowledgments}…}` (no-arg sectioning, NOT the bounded ltx:acknowledgements env). Rust's ieeeaerospace stub (LoadClass IEEEtran) omitted it. Added `\acknowledgments → \section*{Acknowledgments}` (matches cls + Perl output). RUST 1→0. (3) **french `\AddThinSpaceBeforeFootnotes`** (1610.09195): french.ldf L1976 footnote-spacing flag (moot in HTML), family of the existing FDP/auto-spacing no-ops. Added no-op. RUST 1→0. SHARED in same ERR=1 batch: mode-leak `}`, malformed-in-emph, `\SetCustomStyle`, dvisvgm, etc.
* **FIXED 2026-05-29 — catchfile `\CatchFileDef`/`\CatchFileEdef` left target undefined on missing file (root-cause).** Perl catchfile.sty.ltxml does `DefMacroI($_[1], undef, Input($_[2]))` — ALWAYS defines the target CS (Input of a missing file → empty content). Rust's catchfile_sty.rs only def_macro'd the target *inside* `if let Some(disk)…if let Ok(bytes)…`, so a missing/unreadable file left it UNDEFINED. Breaks reading a previous-run aux file absent from the arXiv source: makron.sty L61 `\CatchFileEdef\tmp{\jobname.runs}{…}` then `\setcounter{…}{\tmp}` → `\tmp` undefined where Perl clean. Fix: always `def_macro(target, …)`, empty body when the file can't be read (matches Perl `DefMacroI`). Flips 1611.01359 (stage_62 ERR=1) RUST 1→0. Helps any CatchFile-missing-aux paper. SHARED in same batch: `\bbl@engine` (newer-babel internal, undefined in both bindings, RUST 1/PERL 1).
* **FIXED 2026-05-29 — `\qbezier` space before first paren (`[N] (x,y)…`).** Rust's `\qbezier` spec had `SkipSpaces` before the 2nd/3rd `Match:(` but NOT the 1st, so `\qbezier[10] (247,22)(245,20)(245,10)` (space after `[10]`) failed `expected:Match Missing argument Match:(`. Perl uses `Pair Pair Pair` (LaTeX.pool.ltxml L5182) whose `Pair` skips leading spaces. Fix: add `SkipSpaces` before the 1st `Match:(` (a non-numbered directive, so the body `{#1}{#3}{#4}{#6}{#7}{#9}{#10}` is unchanged — verified picture.xml baseline output identical). Flips 1701.03735 (stage_63 ERR=1, amsart) RUST 1→0. Tests 1344/0. SHARED in same batch: `\scriptsize` (1612.09141, no-`\documentclass` fragment mis-detected as main → no class loaded in BOTH engines), `\}` (1701.01362, RUST 1/PERL 1). SHARED `\}` (1701.01362, RUST 1/PERL 1).
* **FIXED 2026-05-29 (follow-up to above) — `\qbezier` rewritten to `[Number] Pair Pair Pair`, fixing the y3-drop.** The manual `Match:( Until:, Until:)` decomposition silently DROPPED the 3rd control point's Y — the terminal `Until:)` slot returned empty, so `\qbezier(1,2)(3,4)(5,6)` rendered `points="…,0"` (with `[N]` it grabbed unrelated garbage; the picture.xml baseline had enshrined `…138.37,0`). Replaced the whole decomposition with Perl's `[Number] Pair Pair Pair` (LaTeX.pool.ltxml L5182): a DefMacro extracts the 3 `Pair` structs (`ArgWrap::Pair` → `(p.x.0,p.y.0)`) and forwards 6 coords as text to the `\lx@pic@qbezier` constructor (px-scaling kept — separate pre-existing divergence from Perl's raw storage). `Pair` skips leading spaces, so this ALSO subsumes the space-before-paren fix above. Verified: `\qbezier(1,2)(3,4)(5,6)`→`1.38,2.77 4.15,5.53 6.92,8.3` (y3=6 correct); `\qbezier[10] (7,8)(9,10)(11,12)` correct. Regenerated tests/complex…/picture.xml (only the 2 qbezier y3 values changed: `…,0`→`…,110.7`, `…,691.85`→`…,123.15`). Witness 1701.03735 RUST 1→0. Tests 1344/0.
* **FIXED 2026-05-29 — siunitx `per-mode=symbol-or-fraction` (Unknown per-mode error).** `\sisetup{per-mode=symbol-or-fraction}` → `Error:unexpected:symbol-or-fraction Unknown siunitx per-mode` (×3). `six_format_units` (siunitx_sty.rs) handled reciprocal/fraction/symbol but fell through to the catchall for `symbol-or-fraction`. Perl siunitx.sty.ltxml L1062-1063 resolves it up front: `fraction` in display math, `symbol` otherwise. Fix: port that remap via `lookup_font().mathstyle == "display"`. Verified structurally identical to Perl (0-line diff: inline→divide, display→frac). Flips 2110-range 1811.06895 (IEEEtran conf). ALSO corrected the `si_test` baseline (tests/complex/si.xml) — its symbol-or-fraction region had enshrined the buggy catchall flat-product (`joule * power@(mole,-1)…`); regenerated to the now-correct `joule / mole * kelvin`. `cargo test --tests`: 1344 passed, 0 failed. Surfaced via current-binary sweep stage_76.
* **FIXED 2026-05-29 — jmlr2e `\address` undefined (incomplete-binding).** jmlr2e.sty L194 `\def\address#1{\gdef\@address{#1}}` (author institutional address, rendered in the title block). Rust's `jmlr2e_sty.rs` binding intercepts jmlr2e.sty (raw def never runs) and has the other frontmatter macros (`\editor`/`\jmlrheading`) but omitted `\address`; OmniBus only autoloads `\address` for revtex/OmniBus. Perl raw-loads jmlr2e → clean. Fix: `DefMacro \address{} -> \@add@frontmatter{ltx:note}[role=address]` (content-preserving, consistent with the binding). Flips 1711.01660. Surfaced via cortex_worker re-test of stale stage-51-73 CONVERR (most of that set is already-fixed: epstopdf×6, \lx×6, \@inpenc@test, {keywords} all 0 now).
* **FIXED 2026-05-29 — three incomplete-binding clean wins (icml/sagej/svproc).** (1) **icml `\icmlInternship`** (1902.02603): paper does `\printAffiliationsAndNotice{\icmlInternship}`; `\icmlInternship` is defined only in the bundled icml2019.sty L502, never raw-loaded (both engines intercept via icml_support). Perl gobbles `\printAffiliationsAndNotice{}`; Rust preserves the arg as a note (surpass-Perl) → expands the undefined CS. Added generic `\icmlInternship` fallback (matches the existing `\icmlIntershipWork`/`\icmlIntern` markers). RUST 1→0. (2) **sagej `\endnote`/`\theendnotes`** (1901.10968): sagej.cls L108 `\RequirePackage{…,endnotes}`; Perl raw-loads sagej.cls (no Perl binding), Rust's sagej stub intercepts and omitted endnotes. Added `RequirePackage!(endnotes)`. RUST 2→0. (3) **svproc `\frontmatter`/`\backmatter`** (1902.03320): svproc.cls L240-245 defines the matter commands (page-numbering toggles, moot in HTML); binding had `\mainmatter` no-op but not the other two. No-op all three. RUST 1→0. DEFERRED: 1902.04578 (classicthesis+arsclassica `\headmark`/`\pagemark`) is RUST 4 / PERL 2 — not a clean win (Perl also errors). Surfaced via stage_77 complete histogram.
* **FIXED 2026-05-29 — icml `\airesident` (same family as `\icmlInternship`).** icml2019.sty L503 `\newcommand{\airesident}{\textsuperscript{$\dagger$}This work was completed as part of the … AI Residency}`; passed into `\printAffiliationsAndNotice{\icmlEqualContribution\airesident}`. Added generic fallback to icml_support_sty.rs. Flips 1902.09574 (article+icml2019) RUST 1→0. SHARED (skip): 1902.09485 (`\re@DeclareMathSymbol`/`\omicron`, txgreeks[upright] raw-load fails in BOTH engines, RUST 2/PERL 2). Surfaced via stage_78.
* **TRIAGE 2026-05-29 (stage_77/78, all SHARED — no Rust-only win):** (a) `malformed:ltx:XMApp/XMWrap in ltx:emph` cluster (8 papers: 1901.10467, 1812.10958, 1901.03301, …) — all SHARED; either downstream of primary `unexpected:_/^` math-mode errors or both engines place math in `<ltx:emph>` identically (1901.03301 RUST 1=PERL 1). (b) bicaption `expected:{ Missing keyval arguments` (1902.04405, RUST 1/PERL 22) — SHARED: bicaption.sty raw-load fails in BOTH (Perl also hits the keyval at bicaption.sty L161/L381 + `misdefined:#` leaks + `undefined:\bicaption@ifswap`); Rust merely fails less. A full bicaption binding would be a large surpass-Perl effort. (c) `\vspace` undefined (1903.02304, RUST 1=PERL 1) — SHARED: the file is a bare fragment with NO `\documentclass`/preamble, so neither engine loads LaTeX. (d) `\NC@list`/`\prepnext@tok` (1903.05373 revtex+array, RUST 4/PERL 38) — SHARED: both reimplement `\newcolumntype` without array.sty's real column-parsing internals. Net: the fresh-candidate pool in stages 77-78 is now SHARED-dominated; Rust-only wins have thinned as the corpus converged.
* **FIXED 2026-05-29 — siunitx `per-mode=repeated-symbol` + `reciprocal-positive-first` (Unknown per-mode) + `range-units=brackets` arg-loss.** (a) `six_format_units` only dispatched `reciprocal`/`fraction`/`symbol` (+ earlier `symbol-or-fraction`); `repeated-symbol`/`reciprocal-positive-first` fell to the "Unknown per-mode" catchall. Restructured to mirror Perl siunitx.sty.ltxml L1065-1094 exactly: partition numer/denom once, `reciprocal-positive-first` (reorder, per kept) before stripping per, then `fraction`/`repeated-symbol`(per-symbol prefixes EACH denom → `a/b/c`)/`symbol`. Flips 1812.05943 (elsarticle) RUST 1→0. (b) While regenerating the si_test baseline this surfaced a *separate* pre-existing bug: `six_format_range`'s bracketed branch did `range_pres.insert(0, open.unlist().remove(0))` — kept only the open-bracket op's FIRST token, corrupting the dual so `\SIrange[range-units=brackets]{2}{4}{°C}` lost its first arg (`range@([], 4)` vs Perl `range@(2, 4)`; presentation showed a garbage node-counter value). Fixed to prepend the WHOLE op (Perl `unshift`). All 7 si.tex range@ now match Perl byte-for-byte. Regenerated tests/complex/si.xml (m5/m6/m7 regions, all now Perl-faithful, no fragile counter). Surfaced via complete-stage histogram (stage_76).
* **FIXED 2026-05-29 — changes `\deleted` kept content → `\citep` cascade (Perl-parity divergence).** Rust's `changes_sty.rs` expanded `\deleted[]{}`→`#2` (kept deleted text) where Perl's changes.sty.ltxml L25 gobbles it (`Tokens()`). The `changes` `final` option ACCEPTS deletions (text removed); Perl's `{}` param reads the text as an unexpanded group, so inner commands are discarded as raw tokens. Keeping it diverged from Perl, rendered author-deleted text, AND expanded fragile inner commands: witness 1901.02252 (`[final]{changes}`, `\deleted{… \citep{…} …}` under paclic32 which lacks natbib's `\citep`) → `undefined:\citep` + `malformed:ltx:para` cascade. Fix: `\deleted[]{}`→`""` (gobble), exactly matching Perl; `\added`/`\replaced` stay content-preserving (`#2`, also matching Perl). RUST 2→0. Surfaced via current-binary sweep stage_77.
* **FIXED 2026-05-29 — french `\DecimalMathComma`/`\StandardMathComma` undefined (incomplete-binding, faithful no-op).** babel-french/french.ldf L815-877 toggles the math comma's `\mathcode` (punct↔ordinary) — a visual trailing-space nuance. LaTeXML's number tokenizer recognizes digit-comma-digit as one decimal NUMBER regardless of mathcode, so the toggle is an *effective no-op*: Perl (raw-loads french.ldf) produces byte-identical XML WITH vs WITHOUT the call. Honoring the mathcode change literally is harmful (reroutes `,`→family-1 slot→`;`, glyph corruption — caught and reverted). Rust's curated french_ldf skips raw-load → undefined where Perl clean. Fix: `def_macro_noop` both. Witness 1812.03061 (`[francais]{babel}`, revtex4) RUST 1→0, output identical to baseline. Surfaced via current-binary sweep stage_76.
* **FIXED 2026-05-29 — curve2e `\definecolor` undefined (incomplete-stub).** curve2e.sty L16 unconditionally `\RequirePackage{graphicx,color}`. Perl has no curve2e binding → raw-loads the real .sty → runs that `\RequirePackage` → `\color`/`\definecolor` defined. Rust's `curve2e_sty.rs` is a deliberate no-op stub (dodges curve2e's `\the\edef` raw-load crash, witness 1408.2108) that omitted the dependency, so a paper using `\definecolor` WITHOUT its own `\usepackage{color}` (relying on curve2e to supply it) left `\definecolor` undefined where Perl is clean. Fix: add `RequirePackage!(graphicx)` + `RequirePackage!(color)` to the stub (graphicx idempotent). Flips 1810.10468, 1810.10484 (ieeeconf + curve2e), each RUST 1→0. 1408.2108 still 0 errors (no raw-load regression). Surfaced via current-binary sweep stage_75.
* **SHARED (documented, not a target) 2026-05-29 — mdwmath.sty `\sq@readrad` `#`-leak.** `\meaning\sqrtsign` lacks the `"` delimiter the `\sq@readrad#1"#2\#3` macro needs, so the delimited scan over-runs and literal `#` PARAM tokens reach the Stomach (`misdefined:#`) at *load* time. Both engines fail identically (witness 1811.09652 RUST 43 / PERL 44; Perl log shows the same `misdefined:# at mdwmath.sty`). Largest corpus cluster (~1300 papers, mdwtools family) but an upstream LaTeXML `\meaning`-roundtrip limitation. Recorded in `KNOWN_PERL_ERRORS.md`. Also confirmed SHARED this round: `\arrowfill` (1811.10421), `\q_nil` expl3 recursion (1811.11937), `malformed:XMApp/XMArray` math-mode cascades (1811.10827/09659/10886/05156), `\xrightarrow` used without amsmath under revtex4 (1810.08973 — both engines correctly flag it; Rust already cleaner at 1 vs 8).
* **FIXED 2026-05-29 — IEEEtran `\CLASSOPTION<name>true` setters undefined (incomplete-binding).** IEEEtran.cls L232-260 declares each class-option flag via `\newif\ifCLASSOPTION<name>` (creating the conditional AND the `\CLASSOPTION<name>true`/`false` setters); template papers toggle them directly (bare_adv.tex L1014 `\CLASSOPTIONcaptionsofftrue`). Rust's ieeetran_cls.rs only `\let\ifCLASSOPTION<name>\iftrue` for a few options — never creating the setters, and `captionsoff`/`draft`/`final`/`peerreview`/… weren't `\newif`'d at all. Fix: `\newif` all 19 CLASSOPTION flags up front (faithful, defaults false; the option logic still refines values). Flips 1810.05731. `cargo test` 1344/0; control IEEEtran papers (2104.02680, 1808.01933) unchanged. Surfaced via current-binary sweep stage_75.
* **FIXED 2026-05-29 — babel-french `\NoAutoSpacing` undefined (incomplete-binding).** `[frenchb]{babel}` + `\NoAutoSpacing` (french.ldf L506, the French auto-spacing kill-switch). Same family as `\NoAutoSpaceBeforeFDP`/`\FBautospacing` already stubbed no-op in french_ldf.rs (the `:;!?` thin-space is font-language-driven via `\lx@french@punct@*`, not per-paper toggleable; babel `\shorthandoff` is itself a no-op). Rust's curated french.ldf omitted it → undefined where Perl (raw-loads) is clean. Fix: `def_macro_noop("\\NoAutoSpacing")` alongside the FDP toggles. Flips 1810.02869. Surfaced via current-binary sweep stage_75.
* **FIXED 2026-05-29 — babel-french `\frquote` undefined (incomplete-binding).** `french_ldf.rs` skips the french.ldf raw-load (babel-3.x `\SetString` failure) and provides a curated subset that omitted `\frquote` (french.ldf L601, the modern French quotation command). Perl raw-loads french.ldf → defined (`«⁠text⁠»`). Fix: add `\frquote[*]{}` → `\og #1\fg` (single-level guillemets via the existing `\og`/`\fg`; multi-level `\fr@quote` nesting is a visual nuance). Flips 1808.04243 (`[french]{babel}` + `\frquote`). Surfaced via the current-binary sweep's stage_74 failure logs.
* **FIXED 2026-05-29 — asme2ej `{proof}` undefined (eager-preload anti-pattern).** `asme2ej_cls.rs` eagerly loaded amsthm at class-load, so the documented `\let\proof\relax` + later `\usepackage{amsthm}` idiom made amsthm's `\usepackage` a no-op → `\proof` stayed relaxed. Real asme2ej.cls loads neither amsmath/amsthm and defines its own trivlist `\proof` (L1242-1245). Fix: drop eager amsthm preload + port the class `\proof` defs (so proof-without-amsthm papers still render). Flips 2102.03856.
