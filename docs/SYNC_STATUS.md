...
- **babel-greek typesets in LGR under the `luatex` profile** (batch 56jf residual; worker W11 fixing): a plain `U` under `\selectlanguage{greek}` prints ϒ where lualatex keeps TU and prints U. Root: in definitions mode `\input{greek-fontenc.def}` (tuenc-greek.def:34) is skipped as already loaded (Perl the same, Package.pm:2287-2288, :2363), so the TU Greek declarations never happen. Red repro `tools/perfect_kernel/repros/unicode-catcodes/babel_greek_tu_uppercase_acute_u.tex`.
