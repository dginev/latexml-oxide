...
| `math0005251` math-parser hang in AmS-TeX `\align...\endalign` block within `$$...$$` | math0005251 | **OPEN — deferred** (2026-04-30 triage). Bisected: head-200 lines clean, head-210 hangs. Trigger is inside an `\align` AmS-TeX (not amsmath) display-math environment around line 200-210 of source. Process produces no stderr/stdout before SIGTERM at 60s. Likely math-parser similar to 1407.5769 family — deeper investigation needed. |
