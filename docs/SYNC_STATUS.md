...
- [ ] **38. xytest** (65_graphics) — 6 errors, 64 diffs (was 102 TooManyErrors). Session 46: 9 infrastructure fixes: catcode double-finish, at_letter, xy.tex.ltxml overlay, xylatexml SVG driver, serializer namespace prefix, get_node_qname prefix, option @currname fix, namespace reuse for 2nd+ elements. SVG structure correct (svg:svg, svg:g with prefix), extensions load. Remaining: coordinate values zero (range registers not populated by raw TeX), missing svg:foreignObject for embedded math.
...
- [ ] **H. expl3 full loading** — Session 49 (17 commits): 1500→12 errors. All expl3 modules load. Remaining 12 benign errors from `\exp_last_unbraced:NNNNo` at expl3-code.tex line 11527. The expansion chain produces tokens for `\gdef \__file_name_expand_cleanup:w`, but `do_def()` is never called for this CS during loading. Post-load re-definitions in expl3_sty.rs work as workaround. See `tests/expl3_expansion_chain.tex` for minimal reproduction and debugging notes. Key finding: `\tex_gdef:D` reaches `invoke_token` 2213 times during loading, but the specific cleanup:w definition is NOT among the `do_def()` calls. The expansion chain works in isolation (post-load context) but fails during the `load_tex_definitions` processing loop.
