...
- **A file written in memory has no runaway at its end** (`filecontents` / `\openout` files are opened as string mouths, which `read_balanced` crosses): `\edef\y{\@@input f }` over an unbalanced in-memory file swallows the rest of the document and errors at its end (pdflatex: one error at the file's end, the text after it kept). Red repro `tools/perfect_kernel/repros/expansion-primitives/vfs_file_end_runaway.tex`. The mouth type also decides how it is opened (mouth.rs:462-470, :572-578), so the fix needs a separate "is a file level" flag. MED risk.
