...
- [ ] **56. babel suite** (81_babel) — 202 diffs (no longer crashes). Session 46: txtbabel.def + babel_support.sty bindings ported (language→ISO mapping, unicode quote chars, \selectlanguage hook → xml:lang). xml:lang attributes now appear on elements. Remaining: quote chars still `?` fallback (csquotes \AtEndPreamble multilingual setup needs \csq@hook@multilang debugging), section 1 displaced, document xml:lang missing.
