...
- **Text-font state is not synchronised with NFSS** (root-causer 2026-09-25; the pdfTeX-model half of the OT1-garbled-code cluster, ~16 manuals, ~650 hits of “ – ˝ for `\ { }` in code): (1) a class binding's `\small` never runs `\selectfont`, so doc.sty's `\MacroFont` (`\fontfamily\ttdefault … \small`, doc.sty:149-153) never applies cmtt (moloch 141, the frankenstein bundle ~250, moreverb-de, ftnxtra); and plain `\tt`/`\bf` never update `\f@family`/`\f@series`, so a later `\selectfont` resets them (scrartcl `{\tt\small x}` loses typewriter; SHARED with Perl for (1), RUST-ONLY for the second). Fix: sync `\f@family/\f@series/\f@shape` from `merge_font` (a reverse map beside `font::lookup_font_family`) and let a pure size switch run `\selectfont`'s family half (latex.ltx:14107). (2) `\the\font` names the default cmr10 font, not the current one (Perl's "HACK", TeX_Macro.pool.ltxml:272; Rust tex_macro.rs:302-310), so ltxdoc's `\oc@ttf` (ltxdoc.cls:136) and short-math-guide's `\ttfont` print in roman (source2e 221, short-math-guide 25). Fix: `\the\font` returns an identifier carrying the current family/series/shape/size. Both MED risk (every font switch; golden churn). Red repros `tools/perfect_kernel/repros/fonts-nfss/{macrofont_small_selects_tt,old_font_switch_then_size,the_font_names_the_current_font}.tex`.
- **Balanced reads other than definitions do not recover at a file's end** (batch 56ja residual, DIVERGENCES #304): tex.web §339 inserts a `}` for a token-list text (`absorbing`: `\message`, `\write`, `\toks=`, `\expanded`, `\detokenize`, `\unexpanded`) and a `\par` for a macro argument (`matching`); `read_balanced_with_close` recovers only a definition (`is_macrodef`), which also covers `\lstnewenvironment`/`\newtcbinputlisting` bodies that TeX reads as macro arguments. Model the scanner status (defining / matching / absorbing) as a value passed to the read, one guard per status against pdflatex.
- **The case changer splits `\char` from its number and ignores case-change equivalents** (batch 56ja residual): `\MakeUppercase{\>\`α}` under the luatex profile prints `` `”0313Α `` with a "Missing number" warning where lualatex prints `Ὰ`, and `\MakeUppercase{\textalpha{}}` keeps `α` (textalpha.sty:213 `\DeclareCaseChangeEquivalent`). greek-fontenc char-list: 74 warnings. Red repro `tools/perfect_kernel/repros/unicode-catcodes/case_change_greek_accent_char.tex`.
- **Dump gate: `--init=latex.ltx` logs 6 errors** (vendor TL 2025; plain.tex 0): "LaTeX must be made using an initex with no format preloaded" (latex.ltx:~98, `{` already catcode 1), `\sqrtsign` already defined, `\CurrentFile`/`\CurrentFilePath`/`\CurrentFileUsed` already defined, and one more. release-dumps.yml:139-147 records 0 errors on 2026-07-20, so this is a regression somewhere since then (batch 56ix is not the cause: the pre-56ix build gives the same 6 errors and byte-identical dumps). It blocks the next release's zero-error init gate (CLAUDE.md parity rule 4). Bisect running.
- **Other `get_token` reads do not cross an input end either** (tex.web §362: at `scanner_status=normal` every `get_token` crosses a file end, not only `\noexpand`'s): `\expandafter`, `\string`, `\meaning`, `\let`/`\futurelet`, `\afterassignment` at the very end of a file still error or read nothing where TeX takes the next token of the enclosing input. `gullet.rs` `read_token_across_input_ends` (batch 56ix) is the reader to extend them with, one guard per primitive.
- **Auto-opened picture sizes differ from Perl's for text-argument constructors** (batch 56iy sizes an auto-opened `ltx:picture` from its node box, as Perl's tag `afterClose` does; `\put(0,0){Hello}` matches Perl exactly). Perl's `openElementAt` also extends the box of every auto-opened ancestor (`appendNodeBox`, Document.pm:1685-1700; Rust `open_element_at` only sets it, document.rs), and Rust constructors that digest their coordinates as text measure that text. Measured (px, Rust vs Perl): `x \line(1,0){250} y` 57.27×11.61 vs 11.92×8.65; `x \rput(1,1){Hello} y` 43.05×12.3 vs 11.92×8.65; `x \frame{X} y` 111.16×12.3 vs 22.29×12.15. Port `appendNodeBox` into `open_element_at` (every auto-opened node's box changes, so its own batch) and give coordinate-only constructors Perl's box-free arguments.
