...
* **DEFERRED 2026-05-29 — mdwmath `#`-reaches-Stomach (HIGH-IMPACT, SHARED).** The single biggest second-500K failure cluster: 26 of stage_74's 84 CONVERR (×50 stages ≈ ~1300 papers) fail with `Error:misdefined:# (catcode PARAM) should never reach Stomach!` during the raw-load of `mdwmath.sty`. Root: mdwmath's `|`-as-escape catcode trickery (L50 `|def|sq@readrad#1"#2\#3|relax` — `\catcode\`|=0`) for its `\sqrt` machinery; our raw-load's handling of that catcode-regime switch leaks a PARAM `#`. SHARED with Perl (1808.02456: RUST 43 / PERL 44 — Rust marginally better, both fail), so NOT a parity gap — but fixing it would SURPASS Perl on a large cluster (beyond-Perl, serves the 1M-error-free goal). Deep catcode-regime raw-load work, high regression risk — deferred to a dedicated session.
