...
- [ ] **38. xytest** (65_graphics) — 6 errors, 200 diffs (was 102 TooManyErrors). Session 46: 9 infrastructure fixes. Session 50: comprehensive xylatexml.tex.ltxml port (227→700+ lines): `\lx@xy@move@to` with translate, arc-path tips, full `\cirbuild@` with `\R@`, spline/frame/circle/ellipse constructors, `\straight@typeset` RawTeX redirection, spread macros, tip macros, shavespline, CIR macros, `\lx@xy@crv@decipher`. Fixed xylatexml loading (was never loaded — `\xyoption{latexml}` routed through Rust dispatch). Added `Digested::get_dimension()`. **ROOT CAUSE**: xy kernel parsing fails (`\codeof\next@` "command used out of context") — coordinate decoding system doesn't function in our engine. Needs investigation of xy's catcode management and `\codeof` macro.
