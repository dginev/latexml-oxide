...
| `math0005251` math-parser hang in AmS-TeX `\align...\endalign` block within `$$...$$` | math0005251 | **OPEN — deferred, parse-forest blowup** (2026-04-30). Bisected to single line: head-201 clean, head-202 (`\endalign`) hangs. Trigger block in isolation runs cleanly — state-cumulative. **5-min RSS measurement: SIGKILL at 79s wall with 28.7 GB resident, allocation rate ≈380 MB/s** — confirms exponential parse-forest blowup, not slow loop. Concrete fix path: per-equation parse-node cap in `latexml_math_parser/src/parser.rs` with fallback to highest-priority partial parse. The 1407.5769 sibling self-resolved via cumulative recent fixes — math0005251 is on the wrong side of the exponential threshold and needs an explicit cap. |
