...
- [ ] **56. babel suite** (81_babel) — 202 diffs (no longer crashes). Session 46: txtbabel.def + babel_support.sty bindings ported (language→ISO mapping, unicode quote chars, \selectlanguage hook → xml:lang). xml:lang attributes now appear. Root causes found: (1) stray commas from modern babel 3.x preamble processing leaking text into document — NOT from our hooks; (2) `\bbl@main@language` is "nil" (ini-based loading doesn't call `\main@language`) → csquotes can't set quote styles; (3) quote chars `?` fallback. Both issues are babel 3.x compatibility with our TeX engine.
