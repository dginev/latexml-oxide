...
| **xy-pic** package — `\xymatrix`, `\ar` undefined (commutative diagrams) | 1409.7007 | OPEN — `xy.sty` binding gap; non-trivial package. **2026-04-30 investigation:** existing `xy_sty.rs` (264 lines) already does raw-load of `xy.tex` and overlay setup, but bare `\usepackage{xy}` (no options) doesn't auto-load `xymatrix.tex` / `xyarrow.tex` feature modules. Paper expects modern `[all]` defaults. Real fix needs xy option auto-activation tracing — dedicated session work, not a 5-min cycle. Pragmatic alternative: add minimal `\xymatrix{}` / `\ar{...}` placeholder consumers to xy_sty.rs that swallow content and emit `<ltx:ERROR>` markers, eliminating the 112-error cascade at the cost of dropping the actual diagrams. Not yet done; tradeoff between hard error vs lossy stub deferred. |
