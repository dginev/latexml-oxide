...
| # | Closed | Outcome |
|---|---|---|
| **315** | 2026-07-20 (PR) | **Bug (Rust-only; internal-representation leak).** `LookupString("class_options")` returned `"VecDequeStored[a4paper, 12pt]"` — the `Debug` form of the internal enum. Same root value as #314: `class_options` is a `Stored::VecDequeStored`, and `From<&Stored> for String` (store.rs:981) fell through to `format!("{v:?}")` for every non-`String` variant. Fixed by giving `Stored`'s `Display` a `VecDequeStored` arm and routing `From<&Stored> for String` through it, so `LookupString` now returns `"a4paper,12pt"` — the **comma-separated option string**, faithful to Perl `join(',', @options)` / `Explode(join(','))` (`\@classoptionslist`, Package.pm L2457/L2581), and the *same* value `LookupTokens` reverts to (both reversions were corrected in lockstep; #314's landed `"a4paper12pt"` was updated to add the faithful separator). The comma is inserted **only between `String` items** (option lists); Token/Digested items — hook lists like `@at@begin@document`/`afterGroup`, pushed as `$op->unlist` token objects — still concatenate with no separator, so executable token streams stay comma-free. Zero internal blast radius: only the Rhai bindings revert a `String`-item queue (internal option processing reads it via `lookup_vecdeque`; counter `\cl@` lists are `Stored::Tokens`, not queues). Guards: `lookup_string_on_vecdeque_value_does_not_leak_debug_repr` + the updated `lookup_tokens_on_vecdeque_value_does_not_panic`. |
