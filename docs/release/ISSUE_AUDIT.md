...
| # | Closed | Outcome |
|---|---|---|
| **318** | 2026-07-21 (PR) | **Enhancement — external commands from bindings.** BookML shells out to `latexmk`/`dvisvgm` during digestion (needs image size/depth back). Perl `.ltxml` runs `system()` freely; Rhai was sandboxed. Added `Command`, a thin mirror of `std::process::Command` (`Command(program)` + `.arg`/`.args`/`.env`/`.current_dir`/`.output()` → `#{status, success, stdout, stderr}`) so users read the std docs. Runs during digestion (works via #316's `call_deferred_body`). **Security:** a `.rhai` beside an untrusted source auto-loads (`converter.rs::rhai_dispatch`), so command exec is a real RCE surface — **allowed by default** (Perl-faithful) but **blockable** via `LATEXML_DISABLE_SHELL_ESCAPE` (the pdflatex-shell-escape idiom); SAFETY.md updated — any untrusted-input deployment that ships `--features runtime-bindings` must set it (or build without the feature). No hidden per-call timeout (like `system()`); bounded by the conversion wall-time. Guard: `command_output_runs_by_default_and_is_blockable_via_env`. |
