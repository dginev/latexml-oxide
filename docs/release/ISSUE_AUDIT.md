...
| **315** | unclear semantics of `LookupString` in Rhai binding API | documentation, packages | Filed 2026-07-20. Cluster #314–#321. `LookupString("class_options")` leaked the internal `VecDequeStored[…]` Debug repr. PR #325 (comma-separated reversion) was **reverted 2026-07-20** — the comma-join lived in the generic `VecDequeStored` reversion (over-general heuristic); rethinking the reversion representation before re-landing. Parity note: `class_options` is a `TrimmedCommaList` string array in Perl too (`join(',')`, Package.pm:2457), **not** a KeyVals, so a structured-KeyVals store would diverge. |
