...
| `Error:unexpected:&` Stray alignment (17 bundles, annee-scolaire…) | **FIXED batch 25** (2026-09-01): NOT coffins — the align-ledger drifted on l3tl brace-tricks (`\tl_greplace_all` delimited scanning). Root-caused to Perl's retract-on-unread ALIGN_STATE protocol (Perl SHARES the failure, verified); replaced with tex.web's exact protocol (scan-count §342/§357, back_input §325, begin_token_list no-adjust, scan_toks no-localize) + `retract_scanned_braces` for read-token re-emitting closures (\@ifstar family). OXIDIZED_DESIGN #172, KNOWN_PERL_ERRORS #81. Witnesses: zref-check-code 25→0, suanpan-l3 strays 16→0, m1/l3doc 6-line repros 1→0. Guards: `alignment_ledger_expansion_pushback::*`. |
...
| `misdefined:#` (was 111 docs / 2,668 errs) | **Agent-bisected + 3/4 causes FIXED batch 12**: #80 cmdkey cleared the bulk (135-err docs → 1); (C) expl3 half-state normalization — ctex+tikz 1001→0; (A) `\ProvidesExplFile` — latex-via-exemplos repro 94→0; (D) `\@ifnextchar` ## collapse — adtreesdoc 2→0. **Residual = F/E family** (concepts 43, varindex 62, lt3graph 22, underoverlap 82): `\robust@def`-wrapper `#{`/`##` param text scanned from EXPANSION (ltxkeys/catoptions; dry.sty write-out/re-input) — agent dispatched 2026-08-31, tex.web §476 semantics. |
