...
| psmatrix cell template: with the real `\psset` (batch 56x) pst-node's `\psm@endnode` runs and the `\halign` cell template's `\begingroup…\endgroup` pairs desync ("close a group that switched to mode restricted_horizontal" ×4, `\endgroup` non-boxing ×4); pstricks-add's colour keys also read `\pst@getcolor` = xcolor's `\XC@getcolor` (pstricks.sty:155), missing from `xcolor_sty.rs`. | dsptricks/dspTricksManual (100+, capped), pst-eucl-docBG | `graphics-tikz/psmatrix_psk_mnodesize_dsptricks.tex` (psk internals now defined) | OPEN (K9 Stage 3, own batch): pdflatex `\tracinggroups` vs `LXML_TRACE_FRAMES` shows tex.web opening `vbox > align > align > semisimple(u-part) > hbox > semisimple` with the ROW and COLUMN as NEST levels only (§15532 `init_row` push_nest, §15560 `init_col`), while we open four BOXING save frames for the `\halign` (bounded env `bgroup`, `begin_mode`, `start_row` bgroup alignment.rs:420, `start_column` bgroup :453); pst-node's straddling `\psm@beginnode@i` `\hbox\bgroup\begingroup` / `\psm@endnode@i` `\endgroup\egroup` (pst-node.tex:1215/1222) then pops our cell frame, and the template-trailing `\endgroup` (:1187) meets the ROW frame. Fix = row/column boundaries as nest levels invisible to closer matching, the two `\halign` structural groups as non-boxing align-coded frames matched only by the alignment machinery (§15597 `fin_col`, §15743 `fin_align`, §1068 dispatch). Perl 3 (its pst-node binding noops psmatrix; ours re-loads raw pst-node under pstricks_sty.rs:21). Repro `tools/perfect_kernel/repros/kernel-alignment/psmatrix_single_cell_halign_boxing.tex` (`\begin{psmatrix} A \end{psmatrix}`, 2 errors), control: a plain `\vbox{\halign{\begingroup#\endgroup\cr X\cr}}` is green. |
