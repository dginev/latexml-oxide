...
| P52 | `\VerbatimFootnotes` (fancyvrb.sty:33-58, fancybox.sty:647-668): `\let\@footnotetext\V@footnotetext` reads the footnote body LIVE so `\verb`/short-verb catcodes apply | **DONE b55t** | OXIDIZED_DESIGN #192. Implemented dynamic macro indirection on `\footnote` and `\footnotetext` to `\lx@current@footnote`/`text`; the existing fancyvrb/fancybox bindings expose the package-scoped `\VerbatimFootnotes`, which switches to `\lx@vfootnote`/`text` and captures the body live via `\afterassignment`, `capture_body => true`, and a paired `inline_internal_vertical` mode. Guards cover live `\verb`, package scoping, and the nicematrix shortvrb witness; nicematrix errors dropped from 49 to 43. |
