...
| P27 | w3N memoir stub retirement (22 oracle-clean memoir docs erroring, dominant `undefined:\<memoir-CS>`) | **FINAL → b46** | RUST-ONLY obsolete stub (atableau/curve2e/extbook precedent). `memoir_cls.rs` = `LoadClass!("book")` + ~20 geometry noops, hides real `memoir.cls` (`\onelineskip` L62, `vplace` L11305, `\cftbeforechapterskip` L7429, `\HUGE`, `\setsecnumdepth`, `\chapterstyle`, `\xpretocmd`, `\makeoddhead`…). Raw memoir.cls loads **0 errors** with correct `<chapter>/<section>`; titlepages 4→0, dlfltxbmarkup 3→0. Fix: drop registry entry `latexml_contrib/src/lib.rs:451` + `pub mod memoir_cls` (:168), delete file; `load_class` (content.rs:2601/2619) then raw-loads under rawclasses and OmniBus otherwise. Guard: `\documentclass{memoir}\chapter{C}\onelineskip\section{S}` → 0 errors, chapter/section present. Risk LOW-MED (arXiv non-raw memoir → OmniBus, loses geometry noops; rare). Perl ships no memoir.cls.ltxml. |
