...
| 2026-09-06 | — | Correctness item (batch 56z, mode/group frames): a `{` group is digested as a NESTED body (`tex_box.rs` `{` primitive → `digest_next_body(None)`; Perl does the same, TeX_Box.pool.ltxml:30-41 `digestUntil`/`digestNextBody`). Consequence: a bounded body's terminal token that arrives inside a brace group (the `\@startsection@hook` → `\lx@end@abstract` of a `\section` inside an unbalanced `\abstract{`) is invisible to the body's own loop. `LXML_TRACE_TERMINAL=1` shows which loop reads a token. tex.web has one loop (a group is a save-stack level, §1063); making a terminal visible across group levels would mean an until-body that unwinds the groups above it — a deliberate model change, not a patch. |
\n
