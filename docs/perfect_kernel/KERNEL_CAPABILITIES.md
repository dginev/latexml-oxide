...
| 2026-09-09 | K6 | Fourth witness for the `\pdfoutput` persona (root-causer on upmethodology-doc, oracle pdflatex-clean, 1 error): upmethodology-fmt.sty:461-465 picks the figure extension list by `\ifpdf` (`.pdftex_t` vs `.pstex_t`); FindFile/`\IfFileExists` are fine (probe FOUND), only `\ifpdf` differs — `pdftex.rs:11` `\pdfoutput=0` and `iftex_sty.rs:26` tie `\ifpdf` to the luatex profile; iftex.sty:269-292 derives it from `\pdfoutput>0`. Same-host Perl shares (ifpdf.sty.ltxml:19 `\pdffalse`). Fix shape when the persona lands: `\pdfoutput=1` default + `\ifpdf` from output mode (coupled: `\lx@sys@load@backend` reads `\pdfoutput`, l3backend flips dvips→pdftex — faithful), flip guard `ifpdf_is_true_under_the_luatex_profile`; repro `~/data/pk_agents/w22/upmethodology/repros/wtex_ifpdf.tex`. Residue under K6: bxcoloremoji-shortnames, gentombow, scanpages-doc, upmethodology-doc. |
| 2026-09-10 | K6 | User decision: keep Perl parity for now (`\pdfoutput=0`, `\ifpdf` false); the four `\ifpdf`-branch docs stay as documented residue. A per-document `pdf` persona option (harness-keyed on the pdflatex oracle, as `luatex` is) is the recorded shape if this is revisited. |
...
| 2026-09-09 | K9 | Design note (audit item 5, `NumberScan` of batch 56bi): tex.web keeps ONE token-level alignment action in the scanner — §342 `get_next` inserts the `<v_j>` template part when `align_state=0` and a `&`/`\cr` arrives, whatever is scanning (a number scan included); finishing the cell/row is a STOMACH action (§791-799 `fin_col`/`fin_row` in `main_control` on `tab_mark`/`car_ret`). Ours closes the cell in the gullet's `read_x_token` (`check_alignment_data`), so a number scan's lookahead had to be told not to fire it (`NumberScan`). The general shape: the gullet inserts only the v-part (§342), the column/row closing moves to the digestion of the column-end token; `NumberScan` then disappears. HIGH blast (every alignment; the K9 frame ledger), needs its own staged batch with the alignment guards as the bar — recorded, not started. |
...
| 2026-09-20 | K6 | Third persona signal (batch 56eu, `run_doc.sh`): a demonstrated Unicode-font dependency in the pdfTeX run — `undefined:\setmonofont`/`\setmainfont`/`\setsansfont`/`\setmathfont`/`\IfFontExistsTF`/`\directlua`, or luatextra's "LuaTeX is required for this package" — triggers the one-shot luatex retry, but ONLY when the oracle also recorded engine lualatex/xelatex (cleanliness irrelevant). Root cause of the 16 G docs (hvfloat ×11, luatikz, beamerthemeCelestia ×2, asternote, inlinelabel): their lualatex oracle PDF did not build on this host, so the clean-oracle gate ran them as pdfTeX; there libertinus.sty:15-22 / neoschool.cls branch on an HONEST `\iftutex`=false to Type1 paths that never define the font commands — fontspec is never loaded, so there is no engine abort to make faithful (that idea would have broken `fontspec_definers_define_a_font_switch` and flipped nothing). 14 → 0 rng-errors; asternote/inlinelabel (pTeX jlreq + luatexja, PARKED) show no font signal and stay. Rule refined: the retry needs BOTH an unambiguous engine-required signal AND the oracle's engine — either alone re-opens the +78k-error trap (sweep 9); and because it is speculative the harness KEEPS THE BETTER RUN (s107: latex-via-exemplos looped to a TokenLimit Fatal under luatex, pmhanguljamo-kdoc got worse) — a production persona choice needs the same fallback. |
...
| 2026-09-09 | K10 | Generalized (batch 56bn): the byte→character boundary is ONE place (`Tokens` Display = Perl `ToString`), plus TeX's own two (`\csname` in the gullet, the text-run reassembly in the document = the subfont decode); a raw-utf8.def variant was tried and REJECTED (it needs inputenc.sty:79-88's internals and re-emulates TeX; user decision: one Rust decoder + one activation table `install_utf8_byte_activation` + `\u8:` registration is the idiomatic binding); `Token::from` = `str_toks`. Open: items 4-6 of the same review — writable whatsit properties in `before_digest` + `Stored::Opaque` (retires `ENDTRIVLIST_OWN_FRAME`, forest/animate stacks), `NumberScan` → the scanner's `scanner_status`/`align_state`, table-driven auto-open recoveries. |
| 2026-09-09 | K10 | Batch 56bp: the byte→character decoder is SEQUENCE-wise (`decode_byte_run`: valid sequences decode, invalid bytes stay their Latin-1 image, a trailing incomplete sequence stays pending) and the text-run merge decodes only the pending tail (`mouth::byte_mouth_pending_tail`) — the whole-node re-decode per byte was O(n²) (chinesechess: a 2^32-byte String) and mis-read a decoded é (U+00E9, a 3-byte lead's image) directly before a CJK sequence as one invalid run. Guard `byte_mouth_latin1_before_cjk_decodes_sequence_wise`. |
