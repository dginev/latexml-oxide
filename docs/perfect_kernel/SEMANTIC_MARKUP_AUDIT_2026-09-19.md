...
| **`<tags>` misplacement** | ~~algorithm2e (221)~~ **FIXED 56eh+56ei (221→15)** | `element "tags" not allowed here` | **algorithm2e numbered-indented-line rule-before-tags FIXED 56eh** (`\lx@prepend@indentation@` keeps a leading `<tags>` first — RUST-ONLY parity, LaTeXML-block.rnc:189). Residual 20 = double-`\nl` (two tags) + block-end `}`-before-tags, a separate sub-issue |
