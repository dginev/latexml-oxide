...
| Regression check: 30 random papers from April29's 7796 OK bucket on current HEAD (iter 34) | **30 / 30 still clean** — round-17's 21 commits don't break previously-passing papers. |
| April29 severe bucket (`conversion_fatal` ∪ `abort` ∪ `timeout`, 8 papers) on current HEAD | **7 / 8 clean** at default 60s (1204.6266, 1401.0971, 1503.00395, 1506.09203, 1602.06701, math0605199, quant-ph0401134) — only `math0005251` still aborts (rc=137 OOM kill in math-parser hang). |
| April29 filesystem-level hard failures (missing or stub-sized .zip outputs) | **12 papers** before HEAD-fix; **11/12 RESOLVED on current HEAD** (verified 2026-04-30 iter 38). 8 \$ recursion cluster papers fixed by `f6a6175ea`. The other 3 (math0605199, 1506.09203, quant-ph0401134) also now produce valid 314KB-4.4MB HTML with 0 errors — likely cascading benefit of recent fixes; the only persistent hard failure is `math0005251` (architectural math-parser cumulative-state OOM, deferred). Cluster history in `memory/project_lx_dual_recovery_recursion.md`. |
| April29 conversion_error bucket sample (12 of 93 random papers, iter 39) | 2 fully clean (1604.04916, 1810.01436); 10 retain 1-26 errors per paper. Outlier: math0601451 has 1481×`Error:malformed:ltx:XMTok in <ltx:title>` (amsppt+amstex XMath-leaks-into-title pattern, distinct from siunitx XMTok-in-text bug). Other patterns: per-paper stub-undefined CSes (`\gnuplot`, `\bullets` typo for `\bullet`), mode-switch artifacts in abstract/equation contexts. Long-tail per-package work, no shared cluster fix surfaced. |
...
| `\section` undefined post-AMS-class-load | 1012.3836 | **VERIFIED CLEAN 2026-04-30** — 0 errors / 1 warning on current HEAD (verified iter 6). Pending sandbox-rebuild confirmation. |
...
| paper-local `\bullets` | 1312.7418 | **VERIFIED CLEAN 2026-04-30** — converts with no obvious problems on current HEAD (verified iter 6). Pending sandbox-rebuild confirmation. |
| `1407.5769` math-parser infinite loop at page ~71 | 1407.5769 | **SELF-RESOLVED 2026-04-30** — now in April29 sandbox-rebuild `ok` bucket (Status:conversion:1, exit_code 0). Cumulative recent fixes reduced parse-forest fan-out enough to escape the exponential tail. |
| `math0005251` math-parser parse-forest blowup in AmS-TeX `\align` content | math0005251 | **OPEN — Rust regression vs Perl** (2026-04-30). Bisected to head-202 (`\endalign`); state-cumulative across formulas (isolated block converts cleanly). **Perl-vs-Rust comparison on same paper, same `--preload=ar5iv.sty` profile:** Perl converts in **10.5s wall / 234 MB RSS / 0 errors / 7 warnings**; Rust SIGKILL at 79s / 28.7 GB / killed. **Rust is ~130× more memory and 6× slower per formula here** — concrete Marpa-vs-RecDescent perf gap. Per-formula `fresh_engine()` reset bounds memory (28.7 → 8 GB) but doesn't restore parity (still 16 GB at 120s). Real fix needs grammar-level ambiguity reduction (long-horizon: see "math parser open ambiguity hotspots" + `MATH_GRAMMAR_FIRST_PRINCIPLES.md`). Full diagnostic trail in user-memory `project_math_parser_state_cumulative_hangs.md`. |
