...
//!     references are excluded; `\documentclass` / `\documentstyle`
//!     and Mac-classic markers boost the score; bibtex / metafont /
//!     %auto-ignore / withdrawal sentinels disqualify.
//!
...
pub fn find_main_tex(dir: &Path) -> Result<PathBuf, String> {
    ...
    candidates.sort();
    Ok(candidates.into_iter().next().expect("non-empty after filtering"))
}
...
static RE_END_BYE2: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\\(?:end|bye)(?:\s|$)").unwrap());
...
static RE_UUENCODE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^begin \d{1,4}\s+\S+\r?$").unwrap());
