...
fn scoped_assign_lookup_value() {
  ...
  match state.lookup_value("foo") {
    ...
    Some(& ObjectStore::String(ref received_value)) => assert_eq!(received_value, "bar", "global assignment should have value bar"),
    Some(_) => panic!("Looked up value of foo didn't match assignment value")
  };

  state.push_frame();

  state.assign_value("foo", ObjectStore::String("baz".to_string()), Some(Scope::Local));
  match state.lookup_value("foo") {
    None => panic!("Couldn't lookup foo value after assignment"),
    Some(& ObjectStore::String(ref received_value)) => assert_eq!(received_value, "baz", "local assignment should have value baz"),
    Some(_) => panic!("Looked up value of foo didn't match assignment value")
  };


  state.assign_value("foo", ObjectStore::String("overwrite".to_string()), Some(Scope::Local));
  match state.lookup_value("foo") {
    None => panic!("Couldn't lookup foo value after assignment"),
    Some(& ObjectStore::String(ref received_value)) => assert_eq!(received_value, "overwrite", "second local assignment should have value overwrite"),
    Some(_) => panic!("Looked up value of foo didn't match assignment value")
  };
