...
/// Parse a string specification for an LP curry level expression, as expected in YAML notation
/// definitions e.g. `curry: #1 - #2`
pub fn build_spec(declaration: &str) -> Option<CurryTerm> {
  if declaration.is_empty() {
    return None;
  }
  // spec can be a constant, e.g. "1", or an expression that also implies a constraint,
  // e.g. "#1 - #2 + #3" (implied: #1 + #3 > #2, #1 >= 1, #2 >= 1, #3 >= 1)
  let mut lhs = None;
  let mut next_op: Option<char> = None;
  let mut level = 0;
  let mut nested_buffer = String::new();

  for decl_piece in declaration.split_whitespace() {
    let mut decl_iter = decl_piece.chars();
    while let Some(c) = decl_iter.next() {
      // Nested match
      match c {
        '(' => level += 1,
        ')' => {
          level -= 1;
          if level == 0 {
            if let Some(nested_spec) = build_spec(&nested_buffer) {
              match lhs {
                None => lhs = Some(nested_spec),
                Some(lhs_spec) => {
                  lhs = Some(spec_apply(next_op, lhs_spec, nested_spec));
                  next_op = None;
                },
              }
            }
            nested_buffer = String::new();
          }
        },
        c if level > 0 => nested_buffer.push(c),
        _ => match c {
          // Main content match
          '#' => {
            let arg_id = decl_iter.next().unwrap();
            let spec_piece = CurryTerm::Arg(arg_id.to_string().parse::<usize>().unwrap_or(10));
            match lhs {
              None => lhs = Some(spec_piece),
              Some(lhs_spec) => {
                lhs = Some(spec_apply(next_op, lhs_spec, spec_piece));
                next_op = None;
              },
            }
          },
          ...
          '+' => next_op = Some('+'),
          other => {
            // currently we only support single char digits, as literal levels
            let spec_piece = CurryTerm::Literal(
              other
                .to_string()
                .parse::<i8>()
                .unwrap_or_else(|_| panic!("integer is expected, got {}", other)),
            );
            match lhs {
              None => lhs = Some(spec_piece),
              Some(lhs_spec) => {
...
fn spec_apply(op: Option<char>, lhs: CurryTerm, rhs: CurryTerm) -> CurryTerm {
  if let Some(this_op) = op {
    match this_op {
      '-' => CurryTerm::Sub(Box::new(lhs), Box::new(rhs)),
      '+' => CurryTerm::Add(Box::new(lhs), Box::new(rhs)),
      other => panic!("Unknown operator {:?}", other),
    }
  } else {
    panic!("Missing operator! at lhs {:?} and rhs {:?}", lhs, rhs);
  }
}
...
impl CurryConstraints {
  ...
  pub fn drain(&mut self) -> std::collections::hash_set::Drain<CurryConstraint> { self.0.drain() }
