...
LoadDefinitions!({
  ...
  // Perl: LoadFormat('latex') — precompiled latex.ltx state (expl3, fonts, captions).
  // The dump (generated by latexml_oxide --init=latex.ltx) provides definitions from
  // latex.ltx, expl3-code.tex, fontenc, etc.
  //
  // DISABLED: The dump loads 19K+ definitions that conflict with our compiled engine.
  // Needs Phase E work: _loaded flags, load ordering, selective dump loading.
  // The dump IS generated and embedded (resources/dumps/latex.dump.txt via build.rs),
  // but not loaded until the conflict resolution is implemented.
