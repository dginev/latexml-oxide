...
LoadDefinitions!({
  ...
  DefPrimitive!("\\lx@babel@activate@lang@post", {
    ...
    if is_french && lookup_definition(&T_CS!("\\up"))?.is_none() {
      ...
      stomach::digest(Tokenize!(r"\def\primo{1\up{o}}\def\secundo{2\up{o}}"))?;
      stomach::digest(Tokenize!(r"\def\tertio{3\up{o}}\def\quarto{4\up{o}}"))?;
      stomach::digest(Tokenize!(r"\def\fprimo){1\up{o})}\def\fsecundo){2\up{o})}"))?;
      stomach::digest(Tokenize!(r"\def\ftertio){3\up{o})}\def\fquarto){4\up{o})}"))?;
