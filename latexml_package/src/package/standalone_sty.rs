...
LoadDefinitions!({
  ...
  DefPrimitive!("\\@standalone@documentclass OptionalKeyVals {}", sub[(options_kv, class_tks)] {
    bgroup();
    // OXIDIZED_DESIGN #65 (#311): the group just opened is a LaTeXML artifact —
    ...
    // region with the engine's own scope machinery (Perl's `class:`/`counter:`/
    // `label:` idiom, State.pm L680) so `require_package` can give a package
    ...
    // ends exactly when this group pops — we do not have to arrange that. A
    ...
    // sequence") and in Perl — see tests/100_stale_autoload_no_runaway.rs.
