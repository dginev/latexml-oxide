...
LoadDefinitions!({
  ...
  // Perl report.cls.ltxml L33-34: `openbib` injects inline CSS to
  // render bib blocks as display blocks. Port via require_resource on
  // an anonymous Resource — matches article_cls's openbib handler.
  ...
  RequireResource!("ltx-report.css");
...
\@titlepagefalse"
  ...
  // but the counter setup, etc, depends on report
  ...
  // report.cls `\let\@openbib@code\@empty` (article.cls:585) — the default
