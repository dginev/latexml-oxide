...
LoadDefinitions!({
  ...
  DefPrimitive!("\\lx@subfiles@fake@begindocument", {
    ...
    Let!("\\end{document}", "\\lx@subfiles@fake@enddocument", Scope::Global);
  ...
  DefPrimitive!("\\lx@subfiles@fake@enddocument", {
    ...
    if nesting == 0 {
      Let!("\\end{document}", "\\lx@subfiles@saved@enddocument", Scope::Global);
    }
  ...
  let _ = state::push_value("@at@begin@document",
    Tokens!(
      T_CS!("\\lx@subfiles@setup")
    ));
  DefPrimitive!("\\lx@subfiles@setup", {
    AssignValue!("subfiles_nesting" => 0i64, Some(Scope::Global));
    Let!("\\lx@subfiles@saved@enddocument", "\\end{document}", Scope::Global);
    Let!("\\begin{document}", "\\lx@subfiles@fake@begindocument", Scope::Global);
  });
