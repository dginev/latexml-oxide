...
pub fn def_environment(
  ...
  let current_environment_closure = before_digest_simple!({
    assign_value_sym(crate::pin_literal!("current_environment"), env_name.clone(), None);
  ...
  let unexpected_end_closure = after_digest_simple!(_whatsit, {
    let env = lookup_string_from_sym(crate::pin_literal!("current_environment"));
    if env.is_empty() || name_clone != env {
      let message1 = s!("Can't close environment {}", name_clone);
      let message2 = s!(
        "Current are {} ",
        with_stacked_values_sym(crate::pin_literal!("current_environment"), |vals| vals
          .iter()
          .map(|x| s!("{:?}", x))
          .collect::<Vec<String>>()
          .join(", "))
