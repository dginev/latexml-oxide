...
#[thread_local]
static ARENA: Lazy<RefCell<StringInterner<BufferBackend, BuildHasherDefault<FxHasher>>>> =
  Lazy::new(|| {
...
pub fn pin_static(text: &'static str) -> SymStr { ARENA.borrow_mut().get_or_intern_static(text) }

/// Assign a string into the arena, returning a unique symbol associated with it
pub fn pin<S: AsRef<str>>(text: S) -> SymStr { ARENA.borrow_mut().get_or_intern(text) }
...
pub fn len() -> usize { ARENA.borrow().len() }
