...
impl Model {
  ...
  pub fn load_schema(&mut self) -> &Option<String> {
    // Only load once
    if self.schema.is_some() { return &self.schema }
    let name;
    if self.schema_data.is_none() {
      ...
      self.register_namespace("svg".to_string(), Some("http://www.w3.org/2000/svg".to_string()));
      self.register_namespace("xlink".to_string(), Some("http://www.w3.org/1999/xlink".to_string())); // Needed for SVG
      self.register_namespace("m".to_string(), Some("http://www.w3.org/1998/Math/MathML".to_string()));
      self.register_namespace("xhtml".to_string(), Some("http://www.w3.org/1999/xhtml".to_string()));
