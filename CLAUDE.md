...
- **[`docs/LSP_MULTIFILE_PLAN.md`](docs/LSP_MULTIFILE_PLAN.md)** — Project-root + overlay model for the `--server` LSP (**landed 2026-06-04**): every buffer resolves to a detected root (`find_main_tex` / `--whatsin=directory` heuristic, `% !TEX root`), unsaved buffers overlay via the engine's `{file}_contents` channel, per-file diagnostics. Includes implementation-delta notes; live smoke: `tools/lsp_smoke.py`.
