...
- **[`docs/LSP_SERVER.md`](docs/LSP_SERVER.md)** — **Living design/status doc for the `--server` editor LSP** (warm-preamble + fork-body architecture, PR #243 review records, known-gaps worklist). Beyond-Perl product work — kept OUT of `SYNC_STATUS.md` (the parity log). Live smoke: `tools/lsp_smoke.py`.
- **[`docs/LSP_MULTIFILE_PLAN.md`](docs/LSP_MULTIFILE_PLAN.md)** — Project-root + overlay model for the `--server` LSP (**landed 2026-06-04**): every buffer resolves to a detected root (`find_main_tex` / `--whatsin=directory` heuristic, `% !TEX root`), unsaved buffers overlay via the engine's `{file}_contents` channel, per-file diagnostics. Includes implementation-delta notes.
