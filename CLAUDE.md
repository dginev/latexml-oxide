...
- **[`docs/LSP_MULTIFILE_PLAN.md`](docs/LSP_MULTIFILE_PLAN.md)** — Planned project-root + overlay model for the `--server` LSP: resolve every buffer to a detected root (`find_main_tex` / `--whatsin=directory` heuristic, `% !TEX root`), overlay unsaved buffers at the `Mouth::create` seam, per-file diagnostics. Phased; additive to v1.
