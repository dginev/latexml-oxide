...
- **[`docs/performance/ISSUE_361_MEMORY_TIME_PROFILE_2026-07-24.md`](docs/performance/ISSUE_361_MEMORY_TIME_PROFILE_2026-07-24.md)** — very-large **single** document (#361, 232K-line/7.9 MB book, `--splitat=subsection`): full RAM+time diagnosis (peak 9 GB = transient digested boxes + DOM coexisting at Building; flat CPU profile = allocator + libxml2 XPath). **M1+M2 landed** (`List.font`→`Rc<Font>`; box the `DigestedData::KeyVals` variant — together 9.05 → **6.43 GB**, −29 %, at unchanged wall time; per-box size guarded by `digested_data_size_budget`). Pending half: **M3** (stream boxes→DOM in `Document::absorb` to drop peak to ~DOM-only — architectural; distinct from the post-split streaming above). Resume point.
