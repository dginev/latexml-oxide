...
- **[`docs/SOURCE_PROVENANCE.md`](docs/SOURCE_PROVENANCE.md)** — Design for the prioritized beyond-Perl showcase: the **ar5iv-editor** (CodeMirror source ↔ live HTML preview, synced via source locators), plus accurate linting (#47) and Rust-grade author error messages (#92) off the same substrate. Why Perl (LaTeXML#101) stalled and why Rust breaks the deadlock. Locators are opt-in (`--source-map`), off by default.
