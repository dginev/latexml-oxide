...
pub fn normalize_sum_sizes(alignment: &mut Alignment) -> Result<()> {
  ...
  let strut = if let Some(Stored::Dimension(ref d)) = alignment.get_property("strut").as_deref() { *d } else { Dimension::new(0) };
...
//         }
  ...
  alignment.cached_height = Some(Dimension::new(y));    // or account for vertical position of array as a whole)?
