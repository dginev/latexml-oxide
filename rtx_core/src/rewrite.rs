...
impl Rewrite {
  pub fn new(kind:&str, mut options: RewriteOptions) -> Self {
    ...
    if let Some(xpath) = options.select.take() {
      clauses.push(RewriteClause { compiled: true, op: Select, pattern: RewritePattern::String(xpath) })
    }
    // collect the actionable clauses from the options
    if let Some(label) = options.label.take() {
      clauses.push(RewriteClause{compiled: false, op: Label, pattern: RewritePattern::String(label)});
    }
    if let Some(scope) = options.scope.take() {
      clauses.push(RewriteClause{compiled: false, op: Scope, pattern: RewritePattern::Scope(scope)});
    }
    if let Some(xpath) = options.xpath.take() {
      clauses.push(RewriteClause{compiled: false, op: Xpath, pattern: RewritePattern::String(xpath)});
    }
    if let Some(tokens) = options.on_match.take() {
      clauses.push(RewriteClause{compiled: false, op: Match, pattern: RewritePattern::Tokens(tokens)});
    }
    if let Some(replace) = options.replace.take() {
      clauses.push(RewriteClause{compiled: false, op: Replace, pattern: RewritePattern::Closure(replace)});
    }
    if let Some(r) = options.regexp.take() {
      clauses.push(RewriteClause{compiled: false, op: Regexp, pattern: RewritePattern::String(r)});
    }
...
//     if $LaTeXML::DEBUG{rewrite};
  ...
  fn apply_clause(&self, document: &mut Document, tree: &Node, nmatched: usize, mut clauses: VecDeque<&RewriteClause> , state: &mut State) -> Result<()> {
    use RewriteOperator::*;
    ...
    for idx in 0..nmatched { // Remove the nodes to be replaced
    ...
    for ins in inserted.iter() { // Copy the non-semantic parts of font to the replacement
