...
pub fn candidate_pathnames(pathname: &str, options: FindOptions) -> Vec<String> {
  ...
  // TODO: The use of INSTALLDIRS should be rethought entirely, as Rust currently doesn't have a native concept of "installing" a crate and its resources
  //       there either needs to be a more sophisticated build process, OR, a compile step that translates all model/binding dependencies into rust code.
  //       which would allow bundling them side-by-side with the main application.
