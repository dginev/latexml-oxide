...
use definition::{Definition, ConditionalClosure, BeforeDigestClosure, DigestionClosure};
...
//     %traits }, $class; }
  ...
  fn invoke(&self, gullet: &mut Gullet, state: &mut State) -> Result<Tokens> {
    ...
    if let Some(ref cond_type) = self.conditional_type {
      ...
      error!(target: &format!("unexpected:{:?}",self.cs), //$gullet,
...
impl Conditional {
  pub fn invoke_conditional(&self, gullet: &mut Gullet, state: &mut State) -> Result<Tokens> {Ok(Tokens!())}
...
pub fn invoke_else(&self, gullet: &mut Gullet, state: &mut State) -> Result<Tokens> {Ok(Tokens!())}
//   my $stack = $STATE->lookupValue('if_stack');
...
pub fn invoke_fi(&self, gullet: &mut Gullet, state: &mut State) -> Result<Tokens> {Ok(Tokens!())}
//   my ($self, $gullet) = @_;
//   my $stack = $STATE->lookupValue('if_stack');
//   if (!($stack && $$stack[0])) {    # No if stack entry ?
//     Error('unexpected', $LaTeXML::CURRENT_TOKEN, $gullet,
