...
LoadDefinitions!({
  ...
  DefMacro!("\\@tabbing@newline OptionalMatch:* [Dimension]", r"\@tabbing@newline@marker\cr");
  DefMacro!("\\@tabbing@kill", r"\@tabbing@kill@marker\cr\@tabbing@start@tabs");
  ...
  DefMacro!("\\@tabbing@start@tabs", { lookup_tokens("tabbing_start_tabs").unwrap_or_default().unlist() });
  DefPrimitive!("\\@tabbing@increment", {
      let tabs = lookup_tokens("tabbing_start_tabs").unwrap_or_default().unlist();
      assign_value("tabbing_start_tabs" , Tokens!(tabs, T_CS!("\\>")), Some(Scope::Global)); });
  DefPrimitive!("\\@tabbing@decrement", {
      let starts = lookup_tokens("tabbing_start_tabs").unwrap_or_default().unlist();
      let tabs = starts[1];
  ...
  DefPrimitive!("\\@tabbing@bindings", { tabbing_bindings(); });
  // NOTE: Do it!!

  // Internals of tabbing, as an experiment (e.g. files using program.sty raw as in cs/0003026)
  DefMacro!("\\@startfield", r"\global\setbox\@curfield\hbox\bgroup\color@begingroup");
  DefMacro!("\\@stopfield",  r"\color@endgroup\egroup");
  DefMacro!("\\@contfield",  r"\global\setbox\@curfield\hbox\bgroup\color@begingroup\unhbox\@curfield");
  DefMacro!("\\@addfield",   r"\global\setbox\@curline\hbox{\unhbox\@curline\unhbox\@curfield}");
