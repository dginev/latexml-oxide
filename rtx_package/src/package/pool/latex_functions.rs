...
pub fn make_note_tags(counter: &str, mark_opt: &Option<Digested>, tag_opt: Option<Cow<Digested>>, stomach: &mut Stomach, state: &mut State) -> Result<HashMap<String, Stored>> {
  if let Some(tag) = tag_opt {
    ...
    props.insert("tags".to_string(), stomach.digest(
      Tokens!(T_BEGIN!(),
        T_CS!("\\def"), T_CS!(s!("\\the{counter}")), T_BEGIN!(), tag.revert(state)?, T_END!(),
        T_CS!("\\def"), T_CS!(s!("\\typerefnum@{counter}")),
        T_BEGIN!(),     T_CS!(s!("\\{counter}typerefname")), T_SPACE!(), tag.revert(state)?, T_END!(),
        T_CS!("\\lx@make@tags"), T_BEGIN!(), T_OTHER!(counter), T_END!(), T_END!()), state)?.into());
    Ok(props)
  } else {
...
pub fn relocate_footnote(document: &mut Document, node: &mut Node, state: &mut State) -> Result<()> {
  if let Some(caps) = NOTE_TEXT_END.captures(&node.get_attribute("role").unwrap_or_default()) {
    ...
    if let Some(mark) = node.get_attribute("mark") {
      for mut marknote in document.findnodes(&format!(".//ltx:note[@role='{notetype}mark'][@mark='{mark}']"), None, state) {
    ...
    let notetype = caps.get(1).map_or("", |m| m.as_str());    // Eg "footnote", "endnote",...
    if let Some(mark) = node.get_attribute("mark") {
      for mut textnote in document.findnodes(&format!(".//ltx:note[@role='{notetype}text'][@mark='{mark}']"), None, state) {
...
fn relocate_footnote_aux(document: &mut Document, notetype: &str, marknote: &mut Node, textnote: &mut Node, state: &mut State) -> Result<()> {
