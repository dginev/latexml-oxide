...
DefConstructor!("\\lx@add@Preamble@PI Undigested", "<?latexml preamble='#1'?>");
...
DefConstructor!("\\lx@xmarg{}{}", "<ltx:XMArg _xmkey='#1'>#2</ltx:XMArg>",
  ...
  after_digest => sub[stomach,whatsit,state] {
    ...
    if let Some(Stored::HashStored(ref mut pending)) = state.lookup_value_mut("PENDING_DUAL_XMARGS") {
      pending.insert(xmid, arg.into());
    }
    // TODO: Must we store the (currently &mut) Whatsit?
    // let whatsit_stored = Stored::Digested(whatsit.into());
    state.assign_value(&reversion_key, Stored::Tokens(whatsit.revert(state)?), Some(Scope::Global));
    // state.assign_value(&s!("xref:{}@size", xmid), whatsit.get_size(None,state), Some(Scope::Global));
...
DefMacro!("\\@SUPERSCRIPT{}{}", r"\ifx.#2.#1\else\lx@superscript[]{#1}{#2}\fi");
DefMacro!("\\@SUBSCRIPT{}{}",   r"\ifx.#2.#1\else\lx@subscript[]{#1}{#2}\fi");
