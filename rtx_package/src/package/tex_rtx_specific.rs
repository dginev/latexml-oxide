...
LoadDefinitions!(state, {
  ...
  // DefMathI('\lx@ApplyFunction', undef, "\x{2061}", reversion => '', name => '', role =>
  // 'APPLYOP'); DefMathI('\lx@InvisibleTimes', undef, "\x{2062}", reversion => '', name => '',
  // meaning => 'times', role => 'MULOP'); DefMathI('\lx@InvisibleComma', undef, "\x{2063}",
  // reversion => '', name => '', role => 'PUNCT'); DefMathI('\lx@InvisiblePlus', undef,
  // "\x{2064}", reversion => '', name => '', meaning => 'plus', role => 'ADDOP');
  ...
  // DefMacro('\lx@tweaked{}{}',
  // '\ifmmode\lx@math@tweaked{#1}{#2}\else\lx@text@tweaked{#1}{#2}\fi'); DefConstructor('\lx@
  // math@tweaked RequiredKeyVals {}',   "<ltx:XMWrap $XMath_attributes>#2</ltx:XMWrap>",
  //   afterDigest => sub {
  //     my ($stomach, $whatsit) = @_;
