...
//!
//! 1. Stage versioned kernel dumps (`plain.YYYY.dump.txt`, `latex.YYYY.dump.txt`,
//!    `texlive.YYYY.version`) from `resources/dumps/` into `$OUT_DIR`, and emit a manifest
//!    (`embedded_dumps_manifest.rs`) that [`embedded_dumps`](crate::embedded_dumps) `include!`s.
//!    Bundled dumps become the final fallback when no on-disk dump can be resolved (single-file
//!    binary distribution).
//!
//! 2. Emit `latex_dump_loader.rs` into `$OUT_DIR`. The loader resolves a versioned
//!    `latex.YYYY.dump.txt` at runtime, preferring a year that matches the ambient TeXLive (via
//!    [`dump_paths::detect_ambient_texlive_year`](crate::dump_paths)). Fallback chain:
//!
...
// `cargo:rerun-if-changed` path selection for the dump embed. Shared verbatim
// with the library's test suite (`#[cfg(test)] mod build_rerun` in lib.rs); pure
// + std-only so it compiles into this build-script crate with no dependency.
#[path = "src/build_rerun.rs"]
mod build_rerun;
...
fn main() {
  ...
  // The committed `resources/` dir (DTD/Profiles/catalog) — present in every
  // checkout, unlike the gitignored, `make_formats.sh`-generated `dumps/` inside
  // it. We track THIS instead of the possibly-absent `dumps/` (see the emit site).
  let resources_parent = Path::new(&manifest_dir).join("../resources");
  ...
  // registry unpack (whose parent holds sibling crate dirs, no Cargo.toml).
  ...
  //
  // Re-trigger the embed whenever `tools/make_formats.sh` / `release-dumps.yml`
  // writes new dumps — including the FIRST-EVER generation into a not-yet-existing
  // dir. Track the committed `resources/` parent (whose mtime bumps when `dumps/`
  // is created) plus `dumps/` itself once it exists (to catch new dump files, e.g.
  // a new TL year, in the deploy Docker persistent-`target/` cache). NEVER track
  // the dumps dir while absent: Cargo treats a `rerun-if-changed` on a missing
  // path as always-dirty and rebuilds the crate on every invocation — the #676
  // fresh-checkout rebuild loop. Outside a workspace nothing is tracked (the
  // original crates.io #528). Selection logic + guard tests live in build_rerun.
  for p in build_rerun::dump_rerun_paths(
    &resources_parent,
    &dumps_dir,
    in_workspace,
    dumps_dir.exists(),
    ...
    for e in entries.flatten() {
      ...
      for kind in &["plain", "latex"] {
        ...
        if let Some(rest) = name_str.strip_prefix(&prefix)
          && let Some(year_str) = rest.strip_suffix(".dump.txt")
...
"##,
  ...
  // (The dump-directory `rerun-if-changed` set is emitted once, near the top,
  // via `build_rerun::dump_rerun_paths` — do not re-emit the absent dumps dir
  // here; that was the #676 always-dirty rebuild.)
}
...
fn stage_gzipped_or_empty(src: &Path, dst: &Path) {
  ...
  if src.is_file() {
    let raw =
      std::fs::read(src).unwrap_or_else(|e| panic!("Failed to read {}: {}", src.display(), e));
