...
fn main() {
  ...
  if let Ok(entries) = std::fs::read_dir(&dumps_dir) {
    ...
    for year in years {
      ...
      let plain_dst = Path::new(&out_dir).join(format!("embedded_plain.{}.dump.txt", year));
      let latex_dst = Path::new(&out_dir).join(format!("embedded_latex.{}.dump.txt", year));
      let stamp_dst = Path::new(&out_dir).join(format!("embedded_texlive.{}.version", year));
      stage_or_stub(&plain_src, &plain_dst);
      stage_or_stub(&latex_src, &latex_dst);
  ...
  manifest.push_str("  pub plain: &'static str,\n");
  manifest.push_str("  pub latex: &'static str,\n");
  ...
  for (year, plain, latex, stamp) in &manifest_entries {
    ...
    manifest.push_str(&format!("    plain: include_str!({:?}),\n", plain));
    manifest.push_str(&format!("    latex: include_str!({:?}),\n", latex));
...
pub fn load_definitions() -> latexml_core::common::error::Result<()> {{
  ...
  let (content, source_label, stamp_for_check): (String, String, Option<String>) =
    ...
    }} else if let Some((entry, _)) = crate::embedded_dumps::select_embedded(prefer) {{
      log::info!("[latex_dump] using embedded TL{{}} dump (no on-disk dump found)", entry.year);
      let stamp_first = entry.stamp.lines().next().map(|s| s.to_string());
      (entry.latex.to_string(), format!("<embedded TL{{}}>", entry.year), stamp_first)
    }} else {{
